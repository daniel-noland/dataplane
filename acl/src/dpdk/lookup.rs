    ...
    K: MatchKey,
{
    ...
    pub fn lookup_batch<'a>(
        ...
        // `keys.len() <= MAX_BATCH` (guarded above), so neither collect can overflow the
        // ArrayVec capacity; on overflow `collect` panics loudly rather than silently
        // producing a short pointer list.
        let ptrs: ArrayVec<*const u8, MAX_BATCH> = (0..keys.len())
            .map(|i| arena[i * stride..].as_ptr())
            .collect();
        let mut results: ArrayVec<u32, MAX_BATCH> = (0..keys.len()).map(|_| 0u32).collect();
        // SAFETY: every pointer addresses `stride >= min_input_size` valid bytes (invariant
        // established in `new`), packed contiguously above.
        unsafe {
            self.classifier
