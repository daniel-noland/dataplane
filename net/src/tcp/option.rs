...
mod test {
    ...
    fn sack_block_round_trip() {
        bolero::check!()
            ...
            .for_each(|block: &TcpSackBlock| {
                let reconstructed =
                    TcpSackBlock::new(block.left_edge(), block.right_edge());
    ...
    fn sack_accepts_one_to_four() {
        for count in 1..=4 {
            let blocks = vec![TcpSackBlock::new(count, count + 1); count as usize];
            let sack = TcpSack::new(&blocks).unwrap_or_else(|_| unreachable!());
            assert_eq!(sack.blocks().len(), count as usize);
        }
    }
    ...
    fn sack_round_trip() {
        bolero::check!().with_type().for_each(|sack: &TcpSack| {
            ...
            let reconstructed =
                TcpSack::new(blocks).unwrap_or_else(|_| unreachable!());
    ...
    fn option_etherparse_round_trip() {
        bolero::check!().with_type().for_each(|option: &TcpOption| {
            ...
            let back = TcpOption::from_etherparse(&ep)
