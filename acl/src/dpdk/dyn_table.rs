...
use dpdk::acl::{
    ...
    AclField, Built, FieldDef, InvalidAclBuildConfig, InvalidAclName, MAX_FIELDS, Rule, RuleData,
};
...
pub trait DynClassifier: Send + Sync {
    ...
    /// Classify a batch of pre-packed keys in a single `rte_acl` call.
    ///
    /// # Safety
    ///
    /// Every pointer in `data` must address at least
    /// [`min_input_size`](DynClassifier::min_input_size) bytes of valid, initialized memory
    /// laid out according to the ACL context's field definitions (the same per-pointer
    /// contract as [`classify_one`](DynClassifier::classify_one)). `results.len()` must be at
    /// least `data.len()`.
...
impl<A> DynDpdkLookup<A> {
    ...
    pub fn lookup_bytes(&self, key: &[u8]) -> Option<&A> {
        ...
        // SAFETY: stride >= min_input_size (checked in `build_classifier_n`).
