...
fn an_allocation_racing_the_last_release_is_still_served() {
    concurrency::stress(|| {
        let specs = vec![PoolSpec {
            // One address, so a hand-back in flight is the pool's entire capacity. This is the
            // shape the masquerade exposes actually deploy: one public address per peering.
            public_ranges: vec![AddrInterval::new(BASE, BASE)],
            idle_timeout: IDLE_TIMEOUT,
        }];
        let pools = Arc::new(pool_sets_for_specs::<Ipv4Addr>(
