...
impl Icmp6 {
    /// Remap NDP types that etherparse parses as `Unknown`.
    ///
    /// etherparse 0.19 does not have match arms for `RouterSolicitation`
    /// (133), `RouterAdvertisement` (134), or `Redirect` (137) in
...
impl Parse for Icmp6 {
    ...
    fn parse(buf: &[u8]) -> Result<(Self, NonZero<u16>), ParseError<Self::Error>> {
        ...
        // etherparse does not parse RouterSolicitation (133),
        // RouterAdvertisement (134), or Redirect (137) -- they fall
...
mod contract {
    ...
    impl ValueGenerator for Icmp6PacketTooBigGenerator {
        ...
        fn generate<D: Driver>(&self, driver: &mut D) -> Option<Self::Output> {
            ...
            let mtu = Icmp6PacketTooBig::MIN_MTU + driver.produce::<u32>()? % (u32::MAX - Icmp6PacketTooBig::MIN_MTU + 1);
            let icmp_header = Icmpv6Header {
